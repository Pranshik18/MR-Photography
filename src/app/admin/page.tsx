...
import { Plus, Edit2, BookOpen, Layers, Settings } from 'lucide-react';
    ...
    <div className="py-8 md:py-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8 bg-gradient-to-t from-black/80 to-transparent z-10">
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-xs text-neutral-300 mt-1">{recentProjects[0].subtitle}</p>
                  </div>
          ...
          <div className="md:col-span-4 flex flex-col gap-8">
            {recentProjects.slice(1).map((project) => (
              <Link key={project.id} href="/admin/manage-portfolio" className="group relative overflow-hidden flex flex-col cursor-pointer">
      ...
      <section className="mt-14 md:mt-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <Link href="/admin/journal" className="group bg-surface-container-lowest border border-outline-variant/10 hover:border-tertiary/40 rounded-lg p-8 flex flex-col items-start transition-all">
            <div className="w-12 h-12 rounded-full bg-surface-container flex items-center justify-center mb-6 group-hover:bg-tertiary transition-colors">
              <BookOpen className="w-6 h-6 text-outline group-hover:text-[#353025]" />
            </div>
            <h4 className="text-lg font-headline font-bold text-on-surface tracking-tight">Journal</h4>
            <p className="text-xs text-neutral-500 mt-2">Publish and manage editorial entries</p>
          </Link>
